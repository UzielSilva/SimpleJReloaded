...
public class TilesColorSelector extends JPanel implements TilesDataListener,
                                                         ChangeListener {
    ...
    public TilesColorSelector(TilesData tilesData) {
    ...
    public void stateChanged(ChangeEvent e) {
        if (isSelect)
            return;
        tilesData.setColor(colorIndex, redSlider.getValue(),
                          greenSlider.getValue(), blueSlider.getValue());
    }
    
    public void selectIndex(int index) {
        colorIndex = index;
        int v = tilesData.getIAVCValue(index);
        isSelect = true;
    ...
    private class Colors extends JComponent {
        ...
        public void paint(Graphics g) {
            for (int i = 0; i < 16; i++) {
                int x = (i % 4) * 16;
                int y = (i / 4) * 16;
                g.setColor(tilesData.getAWTColor(i));
