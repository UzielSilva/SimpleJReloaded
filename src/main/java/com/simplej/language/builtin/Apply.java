...
public class Apply extends JavaProcedureRef {
    ...
    public void call(ExecutionContext ctx,
        ...
        if (!(o instanceof ProcedureRef))
            throw new NotAProcedureException(ctx);
        ProcedureRef procedureRef = (ProcedureRef) o;
        o = args[1];
        if (!(o instanceof Location[]))
            throw new NotAnArrayException(ctx);
        Location[] largs = (Location[]) o;
        Object[] nargs = new Object[largs.length];
        for (int i = 0; i < nargs.length; i++)
            nargs[i] = largs[i].getValue();
        procedureRef.call(ctx, nargs);
    }
